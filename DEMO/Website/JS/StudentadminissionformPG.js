function continuephase() {
    ...
    $("#infodob").attr("style", "display:none");
    $("#divotpmobile").attr("style", "display:block");
}
function Prevoius1() {
...
function OTP() {
    ...
    $.ajax({
        ...
        data: { MobileNo: stmobileno, Coursetype: 29, sessionid: $("#cseesionid").val() },
        cache: false,
        type: "POST",
        dataType: "json",
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status == true) {
                ...
                $("#infodob").attr("style", "display:block");
                $("#divotpmobile").attr("style", "display:none");
                $("#mobileno").val(stmobileno);
                $("#mobilelable").val(stmobileno);
                $("#mobilelable").html(stmobileno);
            }
            else {
                showAlert(data.msg);
                $("#divotp").attr("style", "display:block");
...
$("#stmobileno").change(function (event) {
    $("#otpverifybutton").attr("style", "display:none");
    $("#OTP").val('');
}); 
   ...
   // var res11 = $("#parmanentcheckbox").check();
    if ($('#parmanentcheckbox').is(":checked")) {
        ...
        if (res == "")
            res = 0;
        $.ajax({
            url: "/StudentPG/HomePG/State_Bind/",
            data: { id: res },
            cache: false,
            type: "POST",
            dataType: "json",
...
function onsave() {
    ...
    //}
    ////if ($("#mothermobile").val() == "") {
    ////    showAlert('Please Enter Mother Mobile !!');
    ////    $("#mothermobile").focus();
    ////    return false;
    ////}
    ////if ($("#motheremail").val() == "") {
    ...
    if (window.FormData !== undefined)
    {
       ...
       var fileData = new FormData();
        ...
        fileData.append('administype1', $("#AdminissionType").val());
        fileData.append('educationtype1', $("#Educationtype").val());
        fileData.append('coursetype1', $("#Coursetype").val());
        ...
        fileData.append('prevoiustreamid', $("#iastream").val());

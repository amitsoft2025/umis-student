...
var app1 = angular.module('downLinelistApp', ['chieffancypants.loadingBar', 'ngAnimate', 'ui.bootstrap']);
app1.config(function (cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = true;
    cfpLoadingBarProvider.latencyThreshold = 500;
});
app1.controller('downLinelistCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.Getdsadownline = function () {
        cfpLoadingBar.start();
        var Searching = $scope.Name.search === undefined ? '' : $scope.Name.search;
             
        $http.get(Url + "api/Getdsadownline?pageSize=" + $scope.pageIndex + "&pageIndex=" + $scope.pageSizeSelected + "&search=" + Searching + "").then(
            function (response) {
                debugger;

var app = angular.module('DueEMIreportlistApp', ['chieffancypants.loadingBar', 'ngAnimate', 'ui.bootstrap']);
app.config(function (cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = true;
    cfpLoadingBarProvider.latencyThreshold = 500;
});

app.controller('DueEMIreportCtrl', function ($scope, $http, cfpLoadingBar) {
    // debugger;
    ...
    $scope.DueEMIList = function () {
        ...
        $http.get(Url + "api/DueEMIreport?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&search=" + Searching + "&customerid=" + scustomerid + "&name=" + sname + "&fromdate=" + sfromdate + "&todate=" + stodate + "&loantypeid=" + LoanSearching + "&loanid=" + loanSubSearching + "&app=" + sapp + "&branch=" + branch + "").then(
            function (response) {
                //debugger;
        ...
        $scope.exportData = function () {
            ...
            var blob2 = new Blob([document.getElementById('dynamicvalue').innerHTML], {
                type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8"
            });

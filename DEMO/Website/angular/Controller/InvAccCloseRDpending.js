...
app.controller('InvaccreportCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.GetHandsIds = function () {
        ...
        var Search = $scope.Search === undefined ? '' : $scope.Search;
        var plantypeSearching = $scope.formSearchtype === undefined ? '' : $scope.formSearchtype;                                               
        $http.get(Url + "api/rdpendinginvaccountreport?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&search=" + Searching + "&searchC=" + SearchingC + "&plantypeid=" + plantypeSearching  + "&date=" + Date + "&dateTo=" + DateTo+ "").then(
            function (response) {
                debugger;
            ...
            $scope.totalCount = response.data.totalCount;
                $scope.exportData = function () {
                    ...
                    //    debugger;
                    saveAs(bb, "PendingRDRenewalReport.xls");
                };
        },
                       function (err) {
                           var error = err;
            });
        //$http.get(Url + "api/PlanName_Bind").then(
        //           function (response) {
    ...
    $scope.GetFilterUsers = function (Code) {
        $scope.GetHandsIds();
    }
    $scope.GetFilterUsersByPlan = function (Search) {
       
        $scope.PlanType_Bind();

    }
    $scope.GetStatusUsers = function (Search) {

...
app.controller('LoanSummaryCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.ClubData = {};
    $scope.jsonToExport = {};
    $scope.Representativecode = "";
    $scope.fromdate = "";
    $scope.todate = "";
    $scope.loandSummaryReport = function () {
        cfpLoadingBar.start();
         debugger;
        var Representativecode1 = $scope.Representativecode === undefined ? '' : $scope.Representativecode;
        var sfromdate = $scope.fromdate === undefined ? '' : $scope.fromdate;
        var stodate = $scope.todate === undefined ? '' : $scope.todate;
        $http.get(Url + "api/investmentpayoutreport?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&Representativecode=" + Representativecode1 + "&fromdate=" + sfromdate + "&toddate=" + stodate + "").then(
            function (response) {
                //debugger;
                ...
                $scope.exportData = function () {
                    ...
                    //    debugger;
                    saveAs(bb, "InvestmentPayoutReport.xls");
                };

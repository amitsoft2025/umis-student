...
app.controller('guestuserlistCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    cfpLoadingBar.start();
        ...
        $http.get(ServiceUrl + "admin/PlanList1?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "").then(
                       function (response) {
                           console.log(response.data.planmasterlists);
                           $scope.employees = response.data.planmasterlists;

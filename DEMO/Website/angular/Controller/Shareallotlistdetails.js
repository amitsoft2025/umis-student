...
app.controller('sharelistCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.GetHandsIds = function () {
        cfpLoadingBar.start();
        $scope.ClubData.search = last_segment;
      //  var ClubData.search=last_segment;
        var Searching = $scope.ClubData.search === undefined ? '' : $scope.ClubData.search;

        $http.get(Url + "api/sharealloetdetailsreport?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&search=" + Searching + "").then(
        function (response) {
            debugger;
            ...
            $scope.exportData = function () {
                ...
                //    debugger;
                saveAs(bb, "ShareAllotmentDetail.xls");
            };
        },
                       function (err) {
                           var error = err;

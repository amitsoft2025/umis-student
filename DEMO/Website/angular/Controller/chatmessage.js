...
app.controller('guestuserlistCtrl', function ($scope, $http, cfpLoadingBar) {
   ...
   // var Url = 'http://localhost:30170/Common/';
    ...
    $scope.getEmployeeList = function () {
        ...
        $http.get(ServiceUrl + "api/chatmessagepaginjg?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "").then(
                       function (response) {
                    
                           console.log(response.data.chatmessagelist);
                           $scope.employees = response.data.chatmessagelist;

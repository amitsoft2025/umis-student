...
function submitapplication() {
    ...
    if ($("#NodalOfficerEmail").val() == "") {
        ...
        if (!filter1.test(email1)) {
            showAlert('Please Enter Valid Nodal Officer Email ID !!');
            $("#NodalOfficerEmail").focus();
            return false;
        }
    }
    ...
    if ($("#PrincipalEmail").val() == "") {
        ...
        if (!filter2.test(email2)) {
            showAlert('Please Enter Valid  Principal Email ID !!');
            $("#PrincipalEmail").focus();
            return false;
        }
    }
    ...
    if (window.FormData !== undefined) {
       ...
       // fileData.append('photo', $("#photo").val());
        ...
        showloader();
        $.ajax({
            url: '/Administrator/Home/AddNewCollage',
            type: "POST",
            ...
            success: function (result) {
                ...
                if (result.status == true) {
                    ...
                    location.replace('/Administrator/Home/ManageCollage');
                    //  showAlert(result);
                }
                else {
                    showAlert(result.Msg);

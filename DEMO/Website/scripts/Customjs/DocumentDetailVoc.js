...
var app1 = angular.module('documentlistApp', ['chieffancypants.loadingBar', /*'ngAnimate',*/ 'ui.bootstrap']);
app1.config(function (cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = true;
    cfpLoadingBarProvider.latencyThreshold = 500;
});
app1.controller('documentlistCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.DocumentdetailList = function () {
        cfpLoadingBar.start();

        $http.get(Url + "api/documentlist?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "").then(
            function (response) {
